...
export default () => {
  ...
  $('#img-control .big').on('click', function () {
    var size =
      imgWH === 'width'
        ? $('#max-img img').width() + 40
        : $('#max-img img').height() + 40;
  ...
  //  给链接设置在新标签页打开
  if (
    $('.post-content').attr('data-target') === '_blank' &&
    $('.post-content a').length
  ) {
  ...
  if ($('.comment-info .author a').length) {
    $('.comment-info .author a').addClass(
      $('.post-content').attr('data-color')
    );
  }

  //  给评论区的取消回复链接添加颜色类
  if ($('#cancel-comment-reply-link').length) {
    $('#cancel-comment-reply-link').addClass(
      $('.post-content').attr('data-color')
    );
  }
  ...
  if ($('body').attr('data-rounded') === 'rounded-0') {
    if ($('.pagination-nav .pagination a').length) {
      $('.pagination-nav .pagination a').addClass(
        $('body').attr('data-rounded')
      );
    }
    if ($('.comment-author .comment-reply a').length) {
      $('.comment-author .comment-reply a').addClass(
        $('body').attr('data-rounded')
      );
    }
  }
  ...
  if ($('.comments-lists .page-navigator').length) {
    ...
    var pageNav =
      '<nav aria-label="分页导航区"><ol class="pagination justify-content-center page-navigator">' +
  ...
  for (
    var i = 0;
    i < $('#emoji-box .emoji-classification button').length;
    i++
  ) {
    //  emoji分类按钮点击
    $('#emoji-box .emoji-classification button')
      .eq(i)
      .on('click', function () {
        if (emoji == null) {
          return false;
        ...
        $('#emoji-box .emoji-classification button').attr(
          'aria-checked',
          false
        ); //  取消所有分类按钮的选中状态（读屏专用）
        ...
        emoji[$(this).attr('classification')].forEach(function (val) {
          emojiDOM +=
            '<div role="listitem" class="float-left emoji" tabindex="0" role="button">' +
            val +
            '</div>';
        });
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (re.test(data)) {
          ...
          $('.post-page').append(
            '<span id="agree-p" role="alert">赞 + 1</span>'
          );
          $('#agree-p').css({
            top: $('#agree-btn').offset().top - 25,
            left:
              $('#agree-btn').offset().left +
              $('#agree-btn').outerWidth() / 2 -
              $('#agree-p').outerWidth() / 2,
          });
  ...
  if ($('#comments .parent').length) {
    ...
    for (var i = 0; i < $('#comments .parent').length; i++) {
      var parentLink =
        ...
        linkColor +
        '" href="' +
        $('#comments .parent').eq(i).attr('href') +
        '">' +
